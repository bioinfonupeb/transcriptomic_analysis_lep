...
DOCKER_VAR["DOCKER_RUN_PREFIX"]="docker run --rm -v ${PATHS["SAMPLE_DIR"]}:/data -v ${PATHS["RAW_DATA"]}:/raw -v
${PATHS["SRC"]}:/src -w /data"
...
run_docker_command() {
  ...
  echo "With Docker run prefix: ${DOCKER_VAR["DOCKER_RUN_PREFIX"]}"
  echo "=========================================================================="
